...
//        $res = $this->db->query($query);
    ...
    public function get_scheduled_channel_for_market($market_name, $internal_ro)
    {
        ...
        $query = "select distinct stc.tv_channel_id,stc.channel_name,sm.id as market_id from sv_advertiser_campaign sac
                       ...
                       where sm.sw_market_name='$market_name' and sac.internal_ro_number='$internal_ro' and sac.campaign_status Not In ('pending_approval','cancelled') and sac.approved_status = 'Approved' order by tv_channel_id ";
        $result = $this->db->query($query);
        if ($result->num_rows() != 0) {
            return $result->result("array");
        }
    ...
    public function get_scheduled_impression_for_time_period($channel_id, $internal_ro, $start_date, $end_date, $market_id = 0)
    {
      ...
      /*  $query = "SELECT (ac.ro_duration*SUM(acsd.impressions )) as total_ad_seconds,acsd.screen_region_id,min(date) as start_date,max(date) as end_date "
        ...
        $query = "SELECT (ac.ro_duration*SUM(acsd.impressions )) as total_ad_seconds,acsd.screen_region_id,min(acsd.date) as start_date,max(acsd.date) as end_date "
        . "FROM sv_advertiser_campaign_screens_dates AS acsd "
        . "Inner Join sv_advertiser_campaign as ac on ac.campaign_id=acsd.campaign_id "
        . "WHERE  ac.channel_id='$channel_id' and (acsd.date >= '$start_date' and acsd.date <='$end_date' ) "
        . "and acsd.screen_region_id in(1,3) and acsd.status ='scheduled' and ac.campaign_status  IN ( 'pending_content', 'saved', 'scheduled', 'completed' ) and approved_status = 'Approved' "
        . "and ac.internal_ro_number='$internal_ro' and ac.is_make_good = 0";
        if(isset($market_id) && !empty($market_id)) {
            $query .= " and ac.market_id='$market_id' " ;
        }
        $query .= " group by acsd.campaign_id;";
        $results = $this->db->query($query);
...
//                        AND internal_ro_number = '$internal_ro'  AND visible_in_easyro = 1 and channel_id = '$channel_id'";
    ...
    public function time_period_proportional_calculation($am_ext_id, $internal_ro, $given_start_date, $given_end_date, $userType, $financial_year)
    {
        ...
        $nw_payout = 0;
        $ro_value_market = 0;
        if(count($market_price) <= 0){
            log_message('INFO', 'In mg_model@time_period_proportional_calculation | $market_price array is empty - ' . print_r($market_price, true));
            return array();
        }
        foreach ($market_price as $mp) {
            ...
            if(count($channel_detail) <= 0){
                log_message('INFO', 'In mg_model@time_period_proportional_calculation | $channel_detail array is empty - ' . print_r($chnl, true));
                    continue;
            }
            foreach ($channel_detail as $chnl) {
                ...
                /*  The code below is commented because we have stopped calculating the total spot ad and banner ad for a span of dates 
                which fall across multiple months . Now even if we get a RO for multiple months , we book a new ro for each month 

                //$total_scheduled_seconds = $this->get_total_scheduled_seconds($internal_ro, $channel_id, $market_id);

                */
                $total_scheduled_seconds = array('spot_ad_seconds' => $scheduled_impression['spot_ad_seconds'] , 'banner_ad_seconds' => $scheduled_impression['banner_ad_seconds']);
...
//        $this->db->query($updateQuery);
    ...
    public function generateFCTMisReport($givenDate, $userType)
    {
        ...
        for ($i = strtotime($current_financial_month_start_date); $i <= strtotime($current_financial_month_end_date); $i = strtotime("+1 month", $i)) {
	        ...
	        /*This condition checks whether its a first date of current month . 
            if its first and in the main loop you have reach just the previous month of the current month , 
            then it will calculate entire previous month till end date of current month otherwise keep the back up data. */ 
	...
	/* commented as not being used anywhere 
