...
class Cache_model extends CI_Model
{
    ...
    public function get_header_menu()
    {
        ...
        if (isset($operation_id)) {
            ...
            $task_detail = $this->get_task_detail_for_operation_id($operation_id);
            if (count($task_detail) > 0) {
                $header_menu = $this->convert_into_header_menu($task_detail);
    ...
    public function get_task_detail_for_operation_id($operation_id)
    {
        $query = "select t.Id,Name,Url from ro_task as t inner join ro_operation_task as ot on t.Id=ot.Task_Fk_Id where ot.Operation_Fk_Id = '$operation_id' and t.Is_Menu='1' and t.Parent_Id is null ";
    ...
    {
        $child_details = $this->get_child_detail_for_parent_id($parent_id);
        $data = array();
    ...
    public function get_child_detail_for_parent_id($parent_id)
    {
