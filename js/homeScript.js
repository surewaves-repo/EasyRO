...
$("document").ready(function() {
      ...
      $.ajax(BASE_URL + "/account_manager/create_ext_ro", {
        ...
        success: function(data) {
          ...
          if (!data.isLoggedIn) {
            ...
            $.getScript("/surewaves_easy_ro/js/create_ext_ro_script.js")
              .done(function() {})
              .fail(function() {});
          }
        },
    ...
    $.ajax(BASE_URL + "/ro_manager/pre_ro_approval/", {
      ...
      success: function(responsedata) {
        ...
        if ("jsonData" in responsedata.Data) {
          ...
          $("#date_camp_start_date").val(
            data.submit_ext_ro_data[0].camp_start_date
          );
          $("#date_camp_end_date").val(
            data.submit_ext_ro_data[0].camp_end_date
          );
          ...
          var customer_release_order =
            ...
            brand_name +
            "</td>" +
            "<td id='approve_camp_start_date'>" +
            ro_start_date +
            "</td>" +
            "<td id='approve_camp_end_date'>" +
            ro_end_date +
            "</td>" +
            "<td>" +
            ro_status +
          ...
          if (data.verify_ro_approved == 0) {
            option =
              option +
              "<span class='link' id='change_ro_dates_span'>Change Ro dates</span>";
          }
          ...
          if (data.scheduled_data.length == 0) {
            ...
            network_summary =
              "<form id='approve_form'><div  class='custom_head'>" +
            ...
            if (typeof data.ro_amount_detail[0].ro_amount == undefined) {
              ...
              if (Number(data.ro_amount_detail[0].ro_valid_field) == 1) {
                network_summary =
                  network_summary +
                  "<div class='col-1 custom_border' style='color:red'><span  id='agency_rebate'>Missing</span></div>" +
            ...
            if (data.logged_in_user.profile_id == 1) {
              ...
              if (data.ro_approval === null) {
                ...
                ro_approval_stat =
                  data.ro_approval[0].ro_approval_request_status == 1;
              }

              if (
                ro_approval_stat ||
                data.ro_amount_detail[0].ro_approval_status == 1
              ) {
                if (
                  data.ro_status_entry == "Approval Requested" &&
                  data.is_cancel_market_requested == 0 &&
                  ...
                  data.ro_status_entry == "Approval Requested" &&
                  data.is_cancel_market_requested == 1
                ) {
                  network_summary =
                    network_summary +
                    "<div class='row'>" +
            ...
            if (
              ...
              network_summary =
                network_summary +
                "<div class='market_priority_div' id='market_priority_div' data-toggle='collapse' data-target='#market_priority_table_div'>" +
                "<h2>Market Priority" +
                "</h2>" +
              ...
              $.map(data.market_priority_details, function(value, index) {
                network_summary =
                  network_summary + "<tr><td>" + value.market_name + "</td>";
                $.map(data.priority, function(val, ind) {
                  if (val in value.channel_priority) {
                    network_summary =
                      network_summary +
                      "<td>" +
                      "<input type='checkbox' class='prioritycheckbox' checked='checked' id='" +
                      value.market_id +
                      "_" +
                      val +
                      "' value='" +
          ...
          $(document).on("click", "#approve_btn", function() {
            ...
            $.map(data.scheduled_data, function(network, nw_index) {
              $.map(network.channels_data_array, function(channel, index) {
                ...
                if (
                  $("#" + nw_index + "_" + channel_id + "_checkbox").attr(
                    "data"
                  ) == 0
                ) {
                  if (channel.total_spot_ad_seconds > 0) {
  ...
  $(document).on("change", ".prioritycheckbox", function(e) {
    ...
    if ($("#" + priority_checkbox_id).is(":checked")) {
      $.map(
        market_priority_detail[market_id].channel_priority[priority_val],
        function(value, index) {
          var channel_checkbox_id = index + "_checkbox";
          if ($("input[id$='" + channel_checkbox_id + "']").length > 0) {
            $.map($("input[id$='" + channel_checkbox_id + "']"), function(
              value
            ) {
              if ($(value).is(":checked")) {
                //nothing
  ...
  $(document).on("click", "#reset_approve_btn", function() {
    ...
    $.map(schedule_data, function(network, nw_index) {
      ...
      $.map(network.channels_data_array, function(channel, index) {
        ...
        $("#" + nw_index + "_" + channel.channel_id + "_checkbox_td").css(
          "background-color",
          "white"
        );
        if (channel.total_spot_ad_seconds > 0) {
          ...
          nw_amount =
            ...
            Number(
              $(
                "#" + nw_index + "_" + channel.channel_id + "_spot_amount"
              ).val()
            );
        }
        if (channel.total_banner_ad_seconds > 0) {
      ...
      total_nw_payout =
        total_nw_payout + Number($("#" + nw_index + "_network_payout").val());
  ...
  $(document).on("change", ".checkboxclass", function(e) {
    ...
    if ($("#" + checkbox_id).is(":checked")) {
      ...
      if (
        $("#" + nw_index + "_" + channel_id + "_total_spot_ad_seconds").length >
        0
      ) {
...
function createApproveData() {
  ...
  $.map(schedule_data, function(network, nw_index) {
    ...
    $.map(network.channels_data_array, function(channel, ind) {
      ...
      if (channel.total_spot_ad_seconds > 0) {
        channel_obj["channel_spot_avg_rate"] = $(
          "#" + nw_index + "_" + channel_id + "_spot_rate"
        ).val();
        channel_obj["channel_spot_amount"] = $(
          "#" + nw_index + "_" + channel_id + "_spot_amount"
        ).val();
        // channel_obj['channel_spot_reference_rate'] = channel.channel_spot_reference_rate;
      }
      if (channel.total_banner_ad_seconds > 0) {
        channel_obj["channel_banner_avg_rate"] = $(
          "#" + nw_index + "_" + channel_id + "_banner_rate"
        ).val();
        channel_obj["channel_banner_amount"] = $(
          "#" + nw_index + "_" + channel_id + "_banner_amount"
        ).val();
      ...
      if (
        $("#" + nw_index + "_" + channel_id + "_checkbox").attr("data") == 0
      ) {
  ...
  if ($("#corrective_action_plan").length > 0) {
    data["corrective_action_plan"] = $("#corrective_action_plan").val();
  }
  if ($("#justification_for_approval").length > 0) {
    data["justification_for_approval"] = $("#justification_for_approval").val();
  }
